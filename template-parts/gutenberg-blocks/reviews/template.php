...
// Prime attachment caches in one pass so each slide's wp_get_attachment_image()
// doesn't trigger its own post/meta queries (avoids N+1).
$image_ids = array_filter(wp_list_pluck($items, 'image_id'));
if ($image_ids) {
...
<section class="<?= esc_attr($block_classes); ?>" <?= $block_anchor ? 'id="'.esc_attr($block_anchor).'"' : ''; ?>>
    ...
    <?php // Instagram icon defined once and referenced from each slide via <use>. ?>
