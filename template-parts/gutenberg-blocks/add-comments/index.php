	...
	acf_register_block_type( array(
		...
		'enqueue_assets'  => static function () {
			...
			if ( file_exists( $style ) ) {
				wp_enqueue_style( 'block-add-comments-style', "{$uri_base}/assets/style.css", array(), filemtime( $style ) );
			}

			$script = __DIR__ . '/assets/script.js';
			if ( file_exists( $script ) ) {
				wp_enqueue_script( 'block-add-comments-script', "{$uri_base}/assets/script.js", array(), filemtime( $script ), true );
			}
