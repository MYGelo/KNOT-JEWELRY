...
// Інвалідуємо кеш preload-блоку коли пост оновлюється
...
/* =========================================================
| CSS FILES LIST CACHE
========================================================= */

// Інвалідуємо кеш списку CSS файлів (glob) при оновленні теми
add_action('upgrader_process_complete', function() {
    delete_transient('knot_css_files_list');
});
...
/* =========================================================
| TAXONOMY TERMS CACHE + HELPER
========================================================= */

// Повертає терміни таксономії з кешу (для фільтр-дропдаунів)
...
// Інвалідуємо кеш таксономій при зміні термінів
add_action('created_term', function($term_id, $tt_id, $taxonomy) {
    delete_transient('knot_tax_terms_' . $taxonomy);
}, 10, 3);
...
// Рендеримо кнопку на сторінці Global Settings
add_action('admin_notices', function() {
    if (!is_admin() || !current_user_can('manage_options')) return;
    if (!isset($_GET['page']) || $_GET['page'] !== 'global_settings') return;
...
// Обробляємо POST від кнопки
add_action('admin_post_knot_clear_all_caches', function() {
    if (!current_user_can('manage_options')) {
        wp_die('Недостатньо прав.');
    }

    if (!isset($_POST['knot_cache_nonce']) || !wp_verify_nonce($_POST['knot_cache_nonce'], 'knot_clear_all_caches')) {
        wp_die('Невірний nonce.');
    }

    global $wpdb;

    // knot_ transients: preload-блок, список CSS файлів, терміни таксономій
    $wpdb->query(
        "DELETE FROM {$wpdb->options}
         WHERE option_name LIKE '_transient_knot\_%'
            OR option_name LIKE '_transient_timeout_knot\_%'"
    );
    ...
    // Кеш відгуків (per-post)
    $wpdb->query(
        "DELETE FROM {$wpdb->options}
         WHERE option_name LIKE '_transient_reviews\_block\_%'
            OR option_name LIKE '_transient_timeout_reviews\_block\_%'"
    );

    // Filter cache — бампаємо версію → всі filter_posts transients стають застарілими
    update_option('filter_cache_version', time());
